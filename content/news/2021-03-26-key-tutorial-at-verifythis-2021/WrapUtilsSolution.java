public class WrapUtils {
    ...
    /*@ public normal_behaviour
      ...
      @
      @  ensures (\forall int a,b; 0 <= a && a < b && b < s.length;
      @    (a == -1 || s[a] == '\n') && s[b] == '\n' ==> b-a >= lineLength);
      @
      @  assignable s[*];
      @*/    
    public static void wrapLines(char[] s, int lineLength) {
        ...
        /*@
          ...
          @    s[i] == \old(s[i]) || \old(s[i]) == ' ' && s[i] == '\n');
          @
          @ loop_invariant (\forall int a,b; 0 <= a && a < b && b < s.length;
          @    s[a] == '\n' && s[b] == '\n' ==> b-a >= lineLength);
          @
...
/* Comment: (*)
  ...
  After the tutorial, a participant asked if that should really read
  from <= s.length.

  Well, it does not hurt. It looks somewhat wrong, but the method just 
  verifies fine.

  It turns out that this is actually required due to the design of the
  wrapLines method that may invoke indexOf with that value s.length 
